			...
			<tbody>
				...
				<c:forEach var="goods" items="${goodsList}">
					<tr>
						...
						<td>
						<c:choose>
								<c:when test="${goods.goods_gubun == 1}">예금</c:when>
								<c:when test="${goods.goods_gubun == 2}">펀드</c:when>
								<c:when test="${goods.goods_gubun == 3}">외환</c:when>
								<c:when test="${goods.goods_gubun == 4}">주식</c:when>
								<c:otherwise>알 수 없음</c:otherwise>
							</c:choose>
						</td>
						<td>${goods.provider_code}</td>
						<td>${goods.goods_name}</td>
						<td>${goods.orign_code}</td>
						<td>${goods.period_mm}</td>
						<td><c:choose>
								<c:when test="${goods.invest_gubun == 1}">주식형</c:when>
								<c:when test="${goods.invest_gubun == 2}">채권형</c:when>
								<c:when test="${goods.invest_gubun == 3}">혼합형</c:when>
								<c:otherwise></c:otherwise>
							</c:choose></td>
						<td>${goods.forignexg_kind}</td>
						<td>${goods.tran_unit}</td>
						<td>${goods.goods_status}</td>
						<td><a class="btn btn-primary btn-rounded btn-sm" href="${pageContext.request.contextPath}/goods/goods_view?goods_code=${goods.goods_code}">보기</a></td>
					</tr>
				</c:forEach>
				<tr>
