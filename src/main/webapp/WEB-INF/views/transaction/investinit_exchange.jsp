...
<script>
        function calculateExchange() {
            ...
            const tranUnit = document.getElementById("tran_unit").value;
            ...
            let trforign_amt = 0; // 이자 변수 초기화
            trforign_amt = Math.ceil((parseFloat(investAmt) / parseFloat(curPrice)) * parseFloat(tranUnit));
 
            // 결과 표시
    		document.querySelector("input[name='tr_number']").value = trforign_amt.toFixed(0); // 소수점 없는 정수로 표시
        }
    </script>    
</head>
...
<form action="${pageContext.request.contextPath}/transaction/investchange" method="post">
...
<input type="hidden" id = "tran_unit" name="tran_unit" value="${transaction.tran_unit}"/>
...
<label for="curprice">환율 :</label>
<input type="text" id="curprice"  name="curprice"  value="${transaction.curprice}" readonly style="background-color: #ffffe0;"/>  원<br/>

<label for="tran_unit">환전단위 :</label>
<input type="text" id="tran_unit"  name="tran_unit"  value="${transaction.tran_unit}" readonly style="background-color: #ffffe0;"/>  <br/>

<label for="invest_amt">투자금액 :</label>
<input type="text" id="invest_amt"    name="invest_amt" />  원
<button type="button" onclick="calculateExchange()">외환 계산하기</button><br/>

<label for="tr_number">매수외환 :</label>
<input type="text"  id="tr_number"   name="tr_number" readonly style="background-color: #ffffe0;"/>  
<label for="forign">${transaction.goods_name}</label>
<br/>
